...
namespace employee.service.Repositories.Database
{
    public class DatabaseRepository : IDatabaseRepository
    {
        ...
        public DatabaseRepository(AppSettings appSettings)
        {
        ...
        public int EditEmployee(int id, string name, string address, int salary, int departmentId)
        {
            int result = 0;
            using (var connection = new SqlConnection(ConnectionString))
            {
                ...
                using (var command = new SqlCommand())
                {
                    CreateProcedureCommand(command, "usp_EditEmployee", connection, sqlParameters.ToArray());
                    result = command.ExecuteNonQuery();
                }
                return result;
            }
